...
- iso-cost method: `tolerance_band` (comparable=`True`)
- target mode: `wae_dynamic`
- baseline envelope size: `4`
- delta acc (wae_dynamic - reference): `-0.0875`
- success(+0.03): `False`
- dominated on (cost, acc): `False` by `[]`
- dominated on (cost, acc, p50lat): `False` by `[]`
- final verdict (dominance-first): `False` (`iso_cost_delta_threshold`)
- plot: `/workspace/wae_router_pilot/runs/round7r2_s1_compare_dynamic_cost_acc_mbpp_eval.png`
...
- delta acc (wae_dynamic - reference): `+0.0000`
- success(+0.03): `False`
- dominated on (cost, acc): `True` by `['wae_dynamic_control_forced_io']`
- dominated on (cost, acc, p50lat): `True` by `['wae_dynamic_control_forced_io']`
- final verdict (dominance-first): `False` (`dominated_by_baseline`)
- plot: `/workspace/wae_router_pilot/runs/round7r2_s1_compare_dynamic_cost_acc_humaneval_eval.png`
- budget best-under-budget:
